...
def _format_pre_computed_facts(method, testable_slices=None):
    ...
    """
    # RQ3 ablation: when the 'facts' component is disabled, emit no pre-computed
    # behavioral facts section at all (the LLM must derive expected values itself).
    if not config.ABLATION.get('facts', True):
        return ""
