...
def run_pipeline(skip_set=None, only=None):
    ...
    for i, method in enumerate(remaining):
        ...
        # ---- RQ3 ABLATION: blank out disabled context components ----
        # Each is computed above, then removed here so a leave-one-out run differs
        # from baseline only in the single disabled component.
        if not config.ABLATION.get('chains', True):
            dep_chain = None          # removes HOW TO CONSTRUCT + abstract-receiver hint everywhere
        if not config.ABLATION.get('callers', True):
            caller_snippets = []      # removes REAL USAGE EXAMPLES
        ...
        allowlist_passed = True   # RQ3 ablation: stays True (gate skipped) when 'allowlist' disabled

        # RQ3 ablation: when 'allowlist' is disabled the whole import + method-call
        # validation/retry gate is bypassed; generated code goes straight to Maven.
        while config.ABLATION.get('allowlist', True):
        ...
        retry_triggered = False
        retry_succeeded = None
        # RQ3 ablation: when 'repair' is disabled there is no Maven compile/runtime
        # retry loop — the first generation result is final.
        max_retries     = config.MAX_RETRIES if config.ABLATION.get('repair', True) else 0
        retry_count     = 0
        ...
        while (not compiled or not passed) and retry_count < max_retries:
            ...
            retry_allowlist_passed = True   # RQ3 ablation: stays True when 'allowlist' disabled
