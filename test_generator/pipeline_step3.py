...
from src.llm_client import call_llm, get_usage_totals, usage_with_cost
...
from src.result_tracker import (load_results, save_result,
                                 is_already_processed, load_usage, save_usage)
...
def run_pipeline(skip_set=None, only=None):
    ...
    truncation_count = 0
    # Cumulative token totals from prior runs (this run's usage is added at the end)
    baseline_usage = load_usage(config.USAGE_JSON)
    print("=" * 40)
    ...
    # Token usage: this run's totals, plus cumulative across all runs (persisted)
    session_usage = get_usage_totals()
    cumulative_usage = usage_with_cost(
        baseline_usage.get('calls', 0)             + session_usage['calls'],
        baseline_usage.get('prompt_tokens', 0)     + session_usage['prompt_tokens'],
        baseline_usage.get('completion_tokens', 0) + session_usage['completion_tokens'],
        baseline_usage.get('reasoning_tokens', 0)  + session_usage['reasoning_tokens'],
        baseline_usage.get('total_tokens', 0)      + session_usage['total_tokens'],
    )
    save_usage(config.USAGE_JSON, cumulative_usage)

    print_final_report(all_results, config.FINAL_REPORT, start_time,
                       truncation_count=truncation_count,
                       usage=session_usage, cumulative_usage=cumulative_usage)
