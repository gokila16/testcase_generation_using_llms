...
# CONFIGURATION
# ============================================================
# Provenance/regeneration script — NOT needed to run the ablation (the inputs it
# produces are already shipped in inputs/). Paths resolve relative to the repo so
# it points at the bundled PDFBox 3.0.5 reactor and writes back into inputs/.
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.dirname(_THIS_DIR)
_PDFBOX   = os.path.join(REPO_ROOT, "pdfbox")   # bundled PDFBox reactor
_INPUTS   = os.path.join(REPO_ROOT, "inputs")

# Every source root that contains classes the test generator may reference.
# The io module (org.apache.pdfbox.io.*) must be included or that package is
# missing from the inventory (RandomAccessReadBuffer, IOUtils, ...).
SOURCE_DIRS = [
    os.path.join(_PDFBOX, "pdfbox", "src", "main", "java"),
    os.path.join(_PDFBOX, "io",     "src", "main", "java"),
]
SOURCE_DIRS = [d for d in SOURCE_DIRS if os.path.isdir(d)]

OUTPUT_FILE = os.path.join(_INPUTS, "class_inventory.json")
